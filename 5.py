...
input_array = np.reshape(np.asarray(lst_CountVec), (int(len(lst_CountVec)/20),20))
...
def w2v(w2v_dim=300) :
    aspect_review = pd.read_csv('data/aspect_review_seg_commaReset20_2.csv')
    #aspect_review = pd.read_csv('data/aspect_review_seg.csv')
    polarity_review = pd.read_csv('data/polarity_review_seg.csv')
    test_review = pd.read_csv('data/test_seg_commaReset20_2.csv')
...
def Review_to_vec(train,test,max_comma) :
    review_train = np.array(train.loc[:,'Review'])
    aspect_vec_train = np.array(train.loc[:,'aspect_vec'])
    review_test = np.array(test.loc[:,'Review'])
    review = np.append(review_train,review_test,axis=0)
    ...
    #pad_sequences(review_train, maxlen=None, dtype='float',
    #padding='pre', truncating='pre', value=0.)

    #lst = []
    lst_CountVec = []
    lst_avtrain = [] # avt means aspect_vec_train
    lst_avtest = []
    index = 0
    for row in review :
        #print(row)
        ...
        for sentence in lst_temp :
            ...
            if lst_temp2 != [''] :
                for word in lst_temp2 :
                    ...
                    #if model_w2v[word] == [] :
                        #print('借殼 is not in vocabulary')
                        continue#w_vec.append#[0.]*300)
                    else :
            ...
            s_vec.append(w_vec)
        if index < 200 :
            lst_avtrain.append(s_vec)
        else :
            lst_avtest.append(s_vec)
        #lst_CountVec = lst_CountVec + lst_temp
        index = index + 1
...
if __name__=='__main__' :
    ...
    epochs = 20
    #w2v(w2v_dim)
    train = pd.read_csv("./data/aspect_review_seg_commaReset20_2.csv")#,header=None,index_col=False)
    train = aspect_to_vec(train)
    #train = comma_reset(train,max_comma)
    test = pd.read_csv("./data/test_seg_commaReset20_2.csv")#,header=None,index_col=False)
    ...
    #test.to_csv('./data/test_seg_commaReset20_2.csv',index=False)
    #train.to_csv('./data/aspect_review_seg_commaReset20_2.csv',index=False)
